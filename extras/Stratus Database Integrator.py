...
if ARGS.players:
	...
	players: list = list(player for player in os.listdir(ARGS.path) if os.path.isfile(os.path.join(ARGS.path, player)))
	players.sort()
	pstats: dict = {}
...
if ARGS.matches:
	...
	matches: list = list(match for match in os.listdir(ARGS.path + "/matches") if os.path.isfile(os.path.join(ARGS.path + "/matches", match)))
	matches.sort()
	mstats: dict = {}
		...
		if match not in matchCache or matchCache[match][0] != mstats[match]["cached"] or matchCache[match][1] is None:
			try:
				...
				if representation == "running" or representation == "finished" or representation == "starting":
					...
					curlRequest("matches/" + match + "/?force-renew")
